...
char *myalloc(int size)
{
  ...
  if (ptr == 0) {
    fprintf(stderr, "Memory allocation bombed.");
  }
  ...
  if (plyfile->file_type == PLY_ASCII) {
    ...
    for (j = 0; j < elem->nprops; j++) {
      ...
      if (prop->is_list) {
        ...
        item = item_ptr[0];
       item_size = ply_type_size2[prop->internal_type];
    ...
    for (j = 0; j < elem->nprops; j++) {
      ...
      if (prop->is_list) {
        item = elem_data + prop->count_offset;
        item_size = ply_type_size2[prop->count_internal];
        get_stored_item ((void *) item, prop->count_internal,
                         &int_val, &uint_val, &double_val);
  ...
  for (type_size = 8; type_size > 0; type_size /= 2) {
    ...
    for (i = 0; i < elem->nprops; i++) {
      ...
      if (prop->is_list) {
        ...
        if (type_size == ply_type_size2[prop->count_external]) {
          prop->count_offset = size;
          size += ply_type_size2[prop->count_external];
        }
      }
      /* not list */
      else if (type_size == ply_type_size2[prop->external_type]) {
        prop->offset = size;
        size += ply_type_size2[prop->external_type];
      }
    }
...
int get_prop_type(char *type_name)
{
  ...
  for (i = PLY_START_TYPE + 1; i < PLY_END_TYPE; i++)
    if (equal_strings (type_name, type_names2[i]))
      return (i);
